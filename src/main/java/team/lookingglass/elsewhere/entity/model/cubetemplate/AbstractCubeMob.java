package team.lookingglass.elsewhere.entity.model.cubetemplate;
...
public abstract class AbstractCubeMob extends AgeableMob {
    ...
    protected static class CubeMobMoveControl<T extends AbstractCubeMob> extends MoveControl {
        ...
        @Override
        public void tick() {
            AbstractCubeMob cubeMob = (AbstractCubeMob) this.mob;
            cubeMob.setYRot(this.rotlerp(cubeMob.getYRot(), this.yRot, 90.0F));
            cubeMob.yHeadRot = cubeMob.getYRot();
            cubeMob.yBodyRot = cubeMob.getYRot();
            if (this.operation != Operation.MOVE_TO) {
                cubeMob.setZza(0.0F);
            } else {
                this.operation = Operation.WAIT;
                if (cubeMob.onGround()) {
                    cubeMob.setSpeed((float) (this.speedModifier * cubeMob.getAttributeValue(Attributes.MOVEMENT_SPEED)));
                    if (this.jumpDelay-- <= 0) {
                        this.jumpDelay = cubeMob.getJumpDelay();
                        if (this.isAggressive) {
                            this.jumpDelay /= 3;
                        }
                        cubeMob.getJumpControl().jump();
                        if (cubeMob.doPlayJumpSound()) {
                            cubeMob.playSound(cubeMob.getJumpSound(), cubeMob.getSoundVolume(), cubeMob.getSoundPitch());
                        }
                    } else {
                        cubeMob.xxa = 0.0F;
                        cubeMob.zza = 0.0F;
                        cubeMob.setSpeed(0.0F);
                    }
                } else {
                    cubeMob.setSpeed((float) (this.speedModifier * cubeMob.getAttributeValue(Attributes.MOVEMENT_SPEED)));
                }
